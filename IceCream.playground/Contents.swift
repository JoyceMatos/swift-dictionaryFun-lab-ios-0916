...
func countForIceCreamFlavor(flavor: String) -> Int {
    ...
    for type in favoriteFlavorsOfIceCream.keys {
        if favoriteFlavorsOfIceCream["\(type)"] == "\(flavor)" {
            quantity.append("\(type)")
            countnumber = quantity.count
        }
    }
    return countnumber
...
let numberOfGuests = favoriteFlavorsOfIceCream.keys.count
...
var attendees1 = numberOfAttendees()
assert(attendees1 == 6, "\(attendees1)")
...
func addPerson(name: String, withFlavor flavor: String) -> Bool {
    ...
    else { favoriteFlavorsOfIceCream["\(name)"] = "\(flavor)"
    return true
}
}
...
attendees1 = numberOfAttendees()
assert(attendees1 == 7, "\(attendees1)")
